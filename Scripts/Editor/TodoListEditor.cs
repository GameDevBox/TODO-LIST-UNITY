...
public class TodoListEditor : EditorWindow
{
    ...
    private void DrawTodoItem(TodoItem item)
    {
        ...
        Color backgroundColor = GetItemBackgroundColor(item);

        // Store the control ID to get the rect after drawing
        ...
        GUILayout.BeginVertical("box");
        {
            ...
            GUILayout.BeginHorizontal();
            {
                // Due date - ALWAYS show in normal color, no red
                string dueDateText = $"Due: {item.dueDate:MMM dd, yyyy}";
                EditorGUILayout.LabelField(dueDateText, GUILayout.Width(120));

                // Created date
                string createdDateText = $"Created: {item.createdDate:MMM dd, yyyy}";
                EditorGUILayout.LabelField(createdDateText, GUILayout.Width(130));

                if (item.estimatedHours > 0)
                {
                    EditorGUILayout.LabelField($"Est: {item.estimatedHours}h", GUILayout.Width(70));
                }
        ...
        // Draw background after the entire item has been rendered
        if (Event.current.type == EventType.Repaint)
        {
            Rect backgroundRect = GUILayoutUtility.GetLastRect();
    ...
    // Add this helper method to get background colors based on status
    private Color GetItemBackgroundColor(TodoItem item)
    {
        return item.status switch
        {
            Status.Completed => new Color(0.15f, 0.15f, 0.15f, 0.3f), // Dark completed
            Status.Blocked => new Color(0.25f, 0.1f, 0.1f, 0.25f),    // Dark red
            Status.OnHold => new Color(0.25f, 0.2f, 0.1f, 0.25f),     // Dark amber
            Status.InProgress => new Color(0.1f, 0.15f, 0.25f, 0.25f), // Dark blue
            Status.NotStarted => new Color(0.2f, 0.2f, 0.2f, 0.2f),   // Medium dark
            _ => new Color(0.2f, 0.2f, 0.2f, 0.2f)
        };
    }

    private GUIStyle GetButtonStyle(Color color)
    {
    ...
    private void DrawSubtasksSection(TodoItem item)
    {
        ...
        GUILayout.BeginVertical("box");
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Add", GUILayout.Width(60)) && !string.IsNullOrEmpty(newSubTask))
                {
